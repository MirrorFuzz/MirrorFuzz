...
def get_semantic_similarity(source_name, target_names, source_dl_framework, target_dl_framework, type):
    ...
    # handle_path = "./results/" + source_dl_framework + "_" + type + "/" + target_dl_framework
...
def calculate_similarity(source_api, all_target_apis, target_apis, source_dl_framework, target_dl_framework,
    ...
    for i in range(len(target_apis)):
        ...
        elif calc_type == "parameter_sim":
            text_similarity = params_cosine_similarities[i]
            semantics_similarity = semantics_params_similarities[i]
    ...
    sorted_similarities = sorted(similarities.items(), key=lambda item: item[1]['final_similarity'], reverse=True)


    top_k_similarities = sorted_similarities[:topk]

    for api_name, similarity in sorted_similarities:
        # if calc_type == "operation_sim":
        #     print(api_name, similarity)
        if similarity['final_similarity'] > h:
            top_k_similarities.append((api_name, similarity))

    # unique_top_k_similarities = list(dict.fromkeys(top_k_similarities))
...
def calc_text_and_sem_sim(source_api,source_dl_framework,target_dl_framework,sim_type):
    ...
    top_k_os_similarities = calculate_similarity(source_api_info, all_target_apis, target_apis, source_dl_framework,
                                              target_dl_framework, sim_type, topk, alpha, os_sim_h, calc_type="operation_sim")
    temp_api_list = {item[0] for item in top_k_os_similarities}

    api_name_list_without_os = [item for item in target_apis if item['name'] not in temp_api_list]
    # print("filter before: ", len(target_apis))
    # print("filter after: ", len(api_name_list_without_os))
    ...
    # if len(ps_result) == 0:
    #     ps_result = os_result
    save_path = os.path.join(current_dir, '.', f"./results/sim_score/{source_dl_framework}/{target_dl_framework}")
    # save_path = f"./results/sim_score/{source_dl_framework}/{target_dl_framework}"
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    json_data = {"operation_sim": os_result, "parameter_sim": ps_result}
    save_file_path = os.path.join(current_dir, '.', f'./results/sim_score/{source_dl_framework}/{target_dl_framework}/{source_api_info["name"]}.json')
